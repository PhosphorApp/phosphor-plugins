...
/// <summary>
/// A lightweight Emby item as returned by the REST API. Only the fields Phosphor needs.
/// </summary>
...
public sealed record EmbyPage(IReadOnlyList<EmbyItem> Items, int TotalCount);

/// <summary>A single chapter marker on an Emby item.</summary>
public sealed record EmbyChapter(string Name, TimeSpan Start);

/// <summary>
/// Pure-<see cref="HttpClient"/> Emby REST client: authenticates a user, browses the media
/// tree (views → artists/albums/tracks, movies, etc.), searches, and builds direct stream URLs.
/// No UI, no threading assumptions — mirrors the shape of the in-box Plex client and the Jellyfin
/// plug-in (Jellyfin is a fork of Emby, so the endpoints are nearly identical).
/// </summary>
public sealed class EmbyClient
{
    // Ticks per 100ns unit → seconds. Emby reports RunTimeTicks in 100-nanosecond units.
    ...
    public EmbyClient(HttpClient http, string deviceId, Action<string>? log = null)
    {
        _http = http;
        _deviceId = deviceId;
        _log = log;
    ...
    /// <summary>
    /// The Emby auth header. Must be exact or the server rejects the request. Version/device
    ...
    public async Task AuthenticateAsync(CancellationToken ct = default)
    {
        ...
        if (!IsConfigured) throw new InvalidOperationException("Emby is not configured.");
        ...
        if (!IsAuthenticated)
            throw new InvalidOperationException("Emby authentication returned no token.");

        _log?.Invoke($"EmbyClient: authenticated as '{_username}' (userId={_userId}).");
    }

    // ── Browse ──────────────────────────────────────────────────────────────────

    /// <summary>The user's top-level views/libraries (Music, Movies, …) via <c>GET /Users/{id}/Views</c>.</summary>
    public async Task<IReadOnlyList<EmbyItem>> GetViewsAsync(CancellationToken ct = default)
    {
    ...
    /// natural children of the parent. When <paramref name="recursive"/> is true the query descends
    /// into nested folders — used for movie/music-video libraries where each title lives in its own
    /// folder, so recursing + filtering to the leaf item type flattens those folders into playable
    /// items (matching the Emby/Jellyfin web clients).
    /// </summary>
    public async Task<EmbyPage> GetItemsAsync(
        string parentId,
        ...
        CancellationToken ct = default)
    {
        ...
        url.Append(recursive ? "&SortBy=SortName&SortOrder=Ascending&Recursive=true"
                             : "&SortBy=SortName&SortOrder=Ascending&Recursive=false");
        url.Append("&Fields=RunTimeTicks,AlbumArtist,Album,PrimaryImageAspectRatio");
        // Emby omits ImageTags from list queries unless images are explicitly requested. Without this
        // albums/artists (folder items) come back with no Primary tag, so their tiles show no art.
    ...
    /// </summary>
    public async Task<IReadOnlyList<EmbyItem>> SearchAsync(
    ...
    // ── Music entity browse (matches the web UI: MusicArtist → MusicAlbum → Audio) ────────────────

    /// <summary>
    /// Lists the <b>MusicArtist</b> entities in a music library. Unlike a raw folder listing, artist
    /// entities carry their own artwork (unique artist images, not derived from a track), so their
    /// tiles show proper art — matching the web client.
    /// </summary>
    public async Task<IReadOnlyList<EmbyItem>> GetArtistsAsync(string parentId, CancellationToken ct = default)
    {
        await AuthenticateAsync(ct);
        var url = $"{_serverUrl}/Artists"
    ...
    /// <summary>
    /// Builds an <c>albumId → primary-image URL</c> map for an artist by scanning the artist's audio
    /// tracks and taking the first track that carries embedded art per album. Emby stores no Primary
    /// image on many <c>MusicAlbum</c> entities (their cover is derived from track art at display
    /// time), so album tiles must borrow a child track's image — this reproduces what the web UI
    /// renders. One query per artist keeps it cheap.
    /// </summary>
    public async Task<IReadOnlyDictionary<string, string>> GetAlbumImageMapAsync(
        string artistId, CancellationToken ct = default)
    {
        ...
        var url = $"{_serverUrl}/Users/{_userId}/Items"
            ...
            + "&Recursive=true"
            + $"&AlbumArtistIds={Uri.EscapeDataString(artistId)}"
            + "&Fields=AlbumId,PrimaryImageAspectRatio"
            + "&EnableImageTypes=Primary&ImageTypeLimit=1";
        using var doc = await GetJsonAsync(url, ct);

        var map = new Dictionary<string, string>();
        foreach (var track in ParseItems(doc.RootElement))
        {
            if (string.IsNullOrEmpty(track.AlbumId)) continue;
            if (map.ContainsKey(track.AlbumId!)) continue;

            // Prefer the track's own embedded Primary; fall back to the album-primary tag it carries.
            var imageUrl = !string.IsNullOrEmpty(track.ImageTag)
                ? GetImageUrl(track.Id, track.ImageTag)
                : (!string.IsNullOrEmpty(track.AlbumImageTag)
                    ? GetImageUrl(track.AlbumId!, track.AlbumImageTag)
                    : null);
            if (imageUrl != null)
                map[track.AlbumId!] = imageUrl;
        }
        return map;
    }

    /// <summary>
    /// Lists the <b>MusicAlbum</b> entities for an artist (by artist id). Album entities carry their
    /// own Primary art, so tiles render correctly.
    /// </summary>
    public async Task<IReadOnlyList<EmbyItem>> GetAlbumsAsync(string artistId, CancellationToken ct = default)
    {
        await AuthenticateAsync(ct);
        var url = $"{_serverUrl}/Users/{_userId}/Items"
            ...
            + "&Fields=RunTimeTicks,AlbumArtist,Album,PrimaryImageAspectRatio"
            + "&EnableImageTypes=Primary&ImageTypeLimit=1";
        using var doc = await GetJsonAsync(url, ct);
        return ParseItems(doc.RootElement);
    }
    ...
    /// <summary>
    /// Lists the <b>Audio</b> tracks of a music album (by album id), ordered by disc/track number.
    /// </summary>
    public async Task<IReadOnlyList<EmbyItem>> GetAlbumTracksAsync(string albumId, CancellationToken ct = default)
    {
        await AuthenticateAsync(ct);
        var url = $"{_serverUrl}/Users/{_userId}/Items"
            + "?IncludeItemTypes=Audio"
    ...
    /// exciters, so we must never emit &gt;2 channels. When <c>stereoAudio</c> is on we ask Emby
    /// for a 2-channel downmix via <c>MaxAudioChannels=2</c> on the stream request. Otherwise we
    /// direct-play.
    /// </summary>
    public string GetStreamUrl(string itemId, bool isAudioOnly)
    {
        ...
        if (isAudioOnly)
        {
            // Direct audio stream endpoint. LibVLC plays these reliably.
            // - No downmix needed → static=true = direct-play/remux the original file untouched.
            // - Stereo (2.1 cab) needed → transcode to a plain 2-channel MP3 stream. A .ts/HLS-style
            //   audio container frequently never reaches LibVLC's "Playing" state and times out, so
            //   we use a self-contained MP3 container (Container=mp3 + AudioCodec=mp3) instead.
            if (_stereoAudio)
            {
                return $"{_serverUrl}/Audio/{itemId}/stream.mp3"
                    + "?static=false"
                    + $"&UserId={_userId}"
    ...
    /// </summary>
    public string? GetBestImageUrl(EmbyItem it)
    {
    ...
    /// Emby reports each chapter's <c>StartPositionTicks</c> (100ns units) and a name. Returns an
    /// empty list when the item has none.
    /// </summary>
    public async Task<IReadOnlyList<EmbyChapter>> GetChaptersAsync(string itemId, CancellationToken ct = default)
    {
        await AuthenticateAsync(ct);
        using var doc = await GetJsonAsync(
            $"{_serverUrl}/Users/{_userId}/Items/{Uri.EscapeDataString(itemId)}?Fields=Chapters", ct);

        var list = new List<EmbyChapter>();
        ...
        foreach (var c in chapters.EnumerateArray())
        {
            ...
            list.Add(new EmbyChapter(name, TimeSpan.FromSeconds(ticks / (double)TicksPerSecond)));
        }
        return list;
    }
    ...
    private EmbyPage ParsePage(JsonElement root)
    {
        var items = ParseItems(root);
        var total = root.TryGetProperty("TotalRecordCount", out var t) && t.TryGetInt32(out var n)
            ? n
            : items.Count;
        return new EmbyPage(items, total);
    }

    /// <summary>Parses an <c>Items</c>-wrapped or bare array of Emby items.</summary>
    private List<EmbyItem> ParseItems(JsonElement root)
    {
    ...
    private static EmbyItem? MapItem(JsonElement e)
    {
        var id = Str(e, "Id");
