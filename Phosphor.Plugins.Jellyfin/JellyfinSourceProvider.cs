...
/// <summary>
/// Provider for the Jellyfin source: browses and plays music/video from a self-hosted Jellyfin
/// server for a logged-in user. Multi-instance (a user may add several servers). Loaded dynamically
/// from the host's <c>plugins/</c> folder.
/// </summary>
public sealed class JellyfinSourceProvider : IPhosphorSourceProvider
{
    public const string JellyfinTypeId = "jellyfin";

    /// <summary>Settings key: Jellyfin server base URL (e.g. http://192.168.1.10:8096).</summary>
    public const string KeyServerUrl = "serverUrl";

    /// <summary>Settings key: Jellyfin account username.</summary>
    public const string KeyUsername = "username";

    /// <summary>Settings key: Jellyfin account password (secret).</summary>
    ...
    public const string ActionBrowseLibraries = "jellyfinBrowseLibraries";

    public string TypeId => JellyfinTypeId;
    public string DisplayName => "Jellyfin";

    public string? Description =>
        "Browses and plays music and video from a self-hosted Jellyfin server. Enter your server " +
        ...
        "Requires a reachable Jellyfin server and account.";
    ...
    [
        new(KeyServerUrl, "Server URL", PluginSettingType.Text,
            HelpText: "Base URL of your Jellyfin server, e.g. http://192.168.1.10:8096"),
        new(KeyUsername, "Username", PluginSettingType.Text,
            HelpText: "Your Jellyfin account username."),
        new(KeyPassword, "Password", PluginSettingType.Secret, Secret: true,
            HelpText: "Your Jellyfin account password. Stored per the host's secret settings."),
    ...
    public IPhosphorSource CreateInstance(string instanceId, IReadOnlyDictionary<string, string?> settings)
        => new JellyfinSource(instanceId, settings);
}
